...
function VideoCallPage() {
    ...
    useEffect(() => {
        const setupSocketListeners = (socket) => {
            ...
            socket.on('peer:left', ({ socketId }) => { setErrorMessage('The other participant has left.'); setCallStatus('ended'); if (remoteVideoRef.current) remoteVideoRef.current.srcObject = null; const pc = peerConnections.current.get(socketId); if (pc) pc.close(); peerConnections.current.delete(socketId); });
        };
